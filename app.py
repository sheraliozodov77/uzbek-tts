...
def find_best_model():
    ...
    # Check multiple possible locations for HF Spaces and local
    possible_paths = [
        Path("output"),  # Local development
        Path("/content/output"),  # Google Colab
        Path("."),  # HF Spaces with model in root
        Path("models"),  # HF Spaces with model in models folder
    ]
    ...
    for output_dir in possible_paths:
        if output_dir.exists():
            ...
            if training_dirs:
                ...
                if best_model_path.exists() and config_path.exists():
                    return best_model_path, config_path
    
    # If no output directory structure found, check for direct model files in root
    direct_model = Path("best_model.pth")
...
def get_model_info():
    ...
    **Model Location**: `{model_path.parent.name if model_path else 'N/A'}`
    """
...
# For Hugging Face Spaces, the interface will be launched automatically
